...
%Task 4 Если задано значение Elem, то предикат записывает в Number номер первого вхождения Elem. 
%Если задано значение Number, то предикат записывает в Elem значение, находящееся в списке под номером Number. 
%Если заданы оба значения, то предикат проверяет, находится ли элемент Elem в списке под номером Number.
...
%Task 5 Программа читает список, читает номер элемента и находит соответствующий элемент. 
...
%Task 12 Удаление элемента с заданным номером; delete_elem(+List, +Number, -Result)
...
%Task 13 Удаление всех элементов, равных данному; delete_all(+List, +Elem, -Result)
delete_all([], _, []):-!.
